...
//Columns to retreive
$aEntity = array('subject', 'fullname', 'statuscode', 'createdon', 'companyname', 'address1_country');
$aEntityLabels = array('subject' => 'Topic', 'fullname' => 'Name', 'statuscode' => 'Status Reason', 'createdon' => 'Created On', 'address1_country' => 'Country/Region');
//View Filters
$viewFilters = array(0 => 'My Open Leads', 1 => 'Open Leads');
//********End Parameters***********************************
		...
		if($result['RetrieveMultipleResult']['BusinessEntities'] != ''){
			...
			foreach ($businessEntitiesList AS $entity){
				...
				<tr>
					...
					<td><?=(!empty($entity['fullname']))? $entity['fullname'] : ''?></td>
					<td><?=(!empty($entity['statuscode']['!name']))? $entity['statuscode']['!name'] : ''?></td>
					<td><?=(!empty($entity['createdon']['!date']))? $entity['createdon']['!date'] : ''?></td>
					<td><?=(!empty($entity['address1_country']))? $entity['address1_country'] : ''?></td>
				</tr>		
				<?
			}
		}else{
