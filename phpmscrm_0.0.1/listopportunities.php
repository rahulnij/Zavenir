...
$szPageTitle = 'Opportunities';
$szEditEntityScript = 'opportunity.php';
...
$statusFilterCondition = 'Open';
//Columns to retreive
$aEntity = array('name', 'customerid', 'estimatedvalue', 'closeprobability', 'actualvalue', 'transactioncurrencyid');
$aEntityLabels = array('name' => 'Topic', 'customerid' => 'Potential Customer', 'estimatedvalue' => 'Est. Revenue', 'closeprobability' => 'Probability', 'actualvalue' => 'Actual Revenue');
//View Filters
$viewFilters = array(0 => 'My Open Opportunities', 1 => 'Open Opportunities');
//********End Parameters***********************************
		...
		if($result['RetrieveMultipleResult']['BusinessEntities'] != ''){
			...
			foreach ($businessEntitiesList AS $entity){
				...
				<tr>
					...
					<td><?=(!empty($entity['customerid']['!name']))? $entity['customerid']['!name'] : ''?></td>
					<td><?=(!empty($entity['estimatedvalue']['!']))? $entity['estimatedvalue']['!']. ' '.currencySymbol($entity['transactioncurrencyid']['!name']) : ''?></td>
					<td><?=(!empty($entity['closeprobability']))? $entity['closeprobability']['!'] : ''?></td>
					<td><?=(!empty($entity['actualvalue']['!']))? $entity['actualvalue']['!']. ' '.currencySymbol($entity['transactioncurrencyid']['!name']) : ''?></td>
				</tr>		
				<?
			}
		}else{
