...
$szPageTitle = 'Activities';
$szEditEntityScript = 'activity.php';
$entityName = 'activitypointer';
$attributeIdName = 'activityid';
//Attribute on which search will be performed
$searchAttributeName = 'subject';
//Attribute on which sorting will be performed
if($sortAttributeName == '')
	$sortAttributeName = 'subject';
//Status to use for Filtered View: Active, Open ...
$statusFilterCondition = 'Open';
//Columns to retreive
$aEntity=array('subject', 'activitytypecode', 'regardingobjectid', 'scheduledstart', 'scheduledend');
$aEntityLabels = array('subject' => 'Subject', 'activitytypecode' => 'Activity Type', 'regardingobjectid' => 'Regarding', 'scheduledstart' => 'Start Date', 'scheduledend' => 'Due Date');
//View Filters
$viewFilters = array(0 => 'My Activities', 1 => 'Open Activities');
//********End Parameters***********************************
		...
		if($result['RetrieveMultipleResult']['BusinessEntities'] != ''){
			...
			foreach ($businessEntitiesList AS $entity){
				...
				<tr>
					...
					<td><?=(!empty($entity['activitytypecode']))? $entity['activitytypecode'] : ''?></td>
					<td><?=(!empty($entity['regardingobjectid']['!name']))? $entity['regardingobjectid']['!name'] : ''?></td>
					<td><?=(!empty($entity['scheduledstart']['!date']))? $entity['scheduledstart']['!date'] : ''?></td>
					<td><?=(!empty($entity['scheduledend']['!date']))? $entity['scheduledend']['!date'] : ''?></td>
				</tr>		
				<?
			}
		}else{
